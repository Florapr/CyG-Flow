...
- Code for the core training/evaluation pipeline is available. Remaining scripts, configs, and assets are being organized and will be uploaded later.
...
BTAD and VisA follow the same train/test split convention; point `--data` to the corresponding root when extending the loader. Full dataset loaders and scripts will be uploaded after cleanup.
...
If you find this work useful, please cite our ICDM Workshop paper (bibtex will be added when the paper link is available):

```bibtex
@inproceedings{florapr_cygflow_icdmw,
  title     = {CyG-Flow: Cyclic Receptive Field Transformation with Gated Multi-scale Normalizing Flows for Industrial Anomaly Detection},
  author    = {florapr},
